...
# Idempotent upsert of versioned CNI provider/employee catalogs on every deploy.
# Disable with SEED_CNI_MASTER_DATA=false if you need to skip it temporarily.
if [ "${SEED_CNI_MASTER_DATA:-true}" != "false" ]; then
