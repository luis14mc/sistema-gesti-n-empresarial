...
async function handler(request: AuthenticatedRequest, { params }: { params: Promise<{ id: string }> }) {
  return runDisposalRoute(request, 'reject', async ({ context, requestId }) => {
    const { id } = await params;
    const { reason } = disposalReasonSchema.parse(await request.json());
    return apiSuccess(await equipmentDisposalService.reject(context, id, reason, requestId), { requestId });
  });
}
