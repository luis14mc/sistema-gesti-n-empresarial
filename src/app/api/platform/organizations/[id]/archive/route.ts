...
async function handler(request: AuthenticatedRequest, context?: { params: { id: string } }) {
  return runPlatformRoute(request, 'archive_organization', async ({ requestId, context: platform }) => {
    const id = context?.params.id;
    if (!id) {
      throw new Error('Organization id is required');
    }
    const body = organizationLifecycleReasonSchema.parse(await request.json().catch(() => ({})));
    const organization = await organizationLifecycleService.archive(
      { organizationId: id, reason: body.reason },
      { ...platform, requestId },
    );
    return apiSuccess({ organizationId: organization.id, status: organization.status, archivedAt: organization.archivedAt }, { requestId });
  });
}
