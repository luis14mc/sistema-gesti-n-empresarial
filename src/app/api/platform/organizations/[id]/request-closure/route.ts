...
async function handler(request: AuthenticatedRequest, context?: { params: { id: string } }) {
  return runPlatformRoute(request, 'request_closure_organization', async ({ requestId, context: platform }) => {
    const id = context?.params.id;
    if (!id) {
      throw new Error('Organization id is required');
    }
    const body = organizationLifecycleReasonSchema.parse(await request.json().catch(() => ({})));
    const organization = await organizationLifecycleService.requestClosure(
