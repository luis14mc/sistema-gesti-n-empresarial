...
import { emitirCompraOrden, getCompraOrden } from '@/lib/compras/orden/service';
...
export const POST = withAuth(async (req: AuthenticatedRequest, { params }) => {
  ...
  if (!canOrdenAction(role, 'emitir', { status: existing.status })) {
    return NextResponse.json({ error: 'Sin permisos' }, { status: 403 });
  }
  try {
    const orden = await emitirCompraOrden(id, req.user!.userId, organizationId);
